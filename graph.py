...
# ========================================================================
# ========================================================================
# LLM 运行模式与模型配置 (默认已升级为 100% 本地完全离线运行版本)
# ========================================================================
# 默认启用本地完全离线模型，保护数据隐私、零 API 成本
USE_LOCAL_MODEL = True  

# 本地 Ollama 推理大模型（完美匹配茶叶蛋本地已有的 deepseek-r1:8b）
LOCAL_CHAT_MODEL = "deepseek-r1:8b"  

# 本地 Ollama 多模态视觉大模型（采用茶叶蛋本地已有的 qwen2.5vl:3b）
...
if USE_LOCAL_MODEL:
    ...
    logger.info(f"RAG 运行模式：【完全本地离线部署】 | 推理模型：'{CHAT_MODEL}' | 视觉模型：'{LOCAL_VISION_MODEL}'")
else:
...
class RAGEngine:
    ...
    def translate_image_to_text(images: List[str]) -> str:
        """将图像翻译成文字描述（本地模式下自动调用本地 Ollama qwen2-vl）"""
        if not images:
            return ""
        
        system_content = "你是一个专业的设备故障分析专家。请仔细观察用户上传的图片，以极其客观、准确、专业的语言描述图片中展示的所有细节、设备型号、指示灯状态、故障现象、以及任何文字标识。描述要求言简意赅，不要产生多余的解释。"
        user_content = "请仔细分析描述这张图片。"

        # 🌟 本地 Ollama 离线解析通道
        if USE_LOCAL_MODEL:
            logger.info(f"正在通过本地 Ollama 【{LOCAL_VISION_MODEL}】进行多模态穿透解析...")
            
            # 清洗 images 列表，Ollama 的 /api/chat 要求传入的必须是纯 Base64（去掉前缀）
            cleaned_images = []
            for img in images:
                if img.startswith("data:image"):
                    # 剥离类似于 "data:image/jpeg;base64," 的前缀
                    parts = img.split(",", 1)
                    if len(parts) > 1:
                        cleaned_images.append(parts[1])
                    else:
            ...
            try:
                resp = requests.post(
                    ...
                    json={
                        ...
                        "messages": [
                            ...
                            {"role": "user", "content": user_content, "images": cleaned_images}
                        ],
                        "stream": False,
                        "options": {
                            "temperature": 0.1
                ...
                desc = resp.json()["message"]["content"]
                logger.info(f"成功通过本地 Ollama 【{LOCAL_VISION_MODEL}】将图片转换为文本描述: {desc[:100]}...")
                return f"\n【用户上传的图片本地解析】：\n{desc}\n"
            except Exception as e:
                logger.error(f"本地多模态模型【{LOCAL_VISION_MODEL}】解析图片失败: {str(e)}。请确保本地已通过命令 `ollama pull {LOCAL_VISION_MODEL}` 成功拉取该模型！")
                return "\n【用户上传了图片，但本地多模态解析通道异常，建议提示用户补充描述图片细节】\n"
    ...
    def call_multimodal_llm(system_prompt: str, user_prompt: str, images: List[str] = None, history: List[Dict[str, str]] = None) -> str:
        ...
        while retries > 0:
            try:
                if USE_LOCAL_MODEL:
                    # 🌟 本地 Ollama 离线路径：强制禁用流式以防流读取假死，并放宽超时至 120 秒给本地 8B 推理模型思考留足空间
                    resp = requests.post(
    ...
    def rerank_documents(query: str, docs: List[Dict[str, Any]], top_n: int = 15) -> List[Dict[str, Any]]:
        ...
        # 🌟 本地离线模式下，直接以 RRF + 领域感知预增益的优质初筛排序返回，100% 避开云端 API 依赖与延迟
        if USE_LOCAL_MODEL:
            for doc in pre_selected:
                doc["score"] = doc["pre_score"]
            logger.info(f"【Rerank 本地降级】已启用 100% 离线 RRF 融合评分，直接截取前 {len(pre_selected)} 篇极高相关度文献返回。")
            return pre_selected
